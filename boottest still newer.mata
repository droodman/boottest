...
class AnalyticalModel { // class for analyitcal OLS, 2SLS, LIML, GMM estimation--everything but iterative ML
	...
	real colvector splus, e, beta, beta0, Ze, ZY
...
class boottestModel {
	...
	real matrix numer, u, U, S, SAR, Splus, Lplus_invRplusLplus, plot, CI, CT_WE
	string scalar wttype, madjtype
	real colvector Dist, DistCDR, s, sAR, plotX, plotY, splus, beta, wtFE
	real rowvector peak
...
void AnalyticalModel::InitEstimate() {
	...
	pragma unset vec; pragma unset val
 	...
 	if (LIML)
		if (parent->el == k) // exactly identified LIML = 2SLS
			...
			if (rows(*pS)) { // includes H0 if pSplus really points to Splus rather than S
				TT = Splus ' TT * Splus
	...
	else if (AR) { // stick to old OLS code for now
		beta0 = *pbetadenom * *pXY
		dbetads = I(rows(beta0)) - *pbetadenom * *pXX
	} else { // OLS
		e0 = *pY - *parent->pXEx * (*pbetadenom * *pXY) // rejigger these to be w.r.t. r0 instead of s, and at this point don't assume r0 scalar
		dedr = *parent->pXEx * ((I(parent->k) - *pbetadenom * *pXX) * Lplus_invRplusLplus)
		
		CT_WE0 = (_panelsum(crosstab(wtFE :* e0  ), clust[BootCluster].info) + _panelsum(crosstab(wtFE :* dedr[|.,.\.,parent->REst|] *parent->pR), clust[BootCluster].info))'
		dCT_WEdr = smatrix(df)
		for (d=df;d;d--)
			dCT_WEdr[d].M = _panelsum(crosstab(wtFE :* dedr[,parent->REst+d]), clust[BootCluster].info)'
	}
}
...
void boottestModel::boottest() {
	real colvector rplus, numer_l, _e, IDExplode, Ystar, _beta, betaEnd, wt, sortID, o, _FEID
	real rowvector val, YstarYstar
	real matrix betadev, Rplus, L, Lplus, vec, combs, t, ZExclYstar, XExYstar, Subscripts, Zi, AVR0, eZVR0, eu, VR0, infoAll, IDCollapse, XExi, QQ, SewtXV
	...
	if (!initialized) {  // for efficiency when varying r0 repeatedly to make CI, do stuff once that doesn't depend on r0
		...
		if (ML) 
			...
			Rplus = REst? *pR \ *_pR0 : *_pR0 // combine model and hypothesis constraints to prepare to "invert" them as a group too
			if (rows(Rplus)) {
				Lplus = invsym(Rplus * Rplus')
				if (!all(diagonal(Lplus)))
					_error(111, "A null hypothesis constraint is inconsistent or redundant.")
				symeigensystem(Rplus ' Lplus * Rplus, vec, val)
				Lplus  = vec[|.,. \ .,rows(Rplus)|]
				Splus = rows(Rplus) < cols(vec)? vec[|.,rows(Rplus)+1 \ .,.|] : J(rows(vec), 0, 0)
				Lplus_invRplusLplus = Lplus * luinv(Rplus * Lplus)
			} else
				Splus = J(0,0,0)
			...
			M_DGP.SetS(Splus) // (potentially) constrained model in DGP; Splus imposes constraints, pR0 tests hypotheses on results
			M_DGP.InitEstimate()
	...
	if (!ML) // GMM, 2SLS, analytical LIML
		if (AR) {
			...
			pM_Repl->Estimate(sAR) // *** need to change to rAR somehow
			pM_Repl->InitTestDenoms(SAR)
		} else if (null | !rows(M_DGP.beta)) { // don't need to recompute if we're not actually imposing the null
			rplus = null? *pr0 : J(0, 1, 0); if (REst) rplus =  *pr \ rplus // constant terms of model + null constraints
			splus = rows(rplus) ? Lplus_invRplusLplus * rplus : J(0,1,0)
			M_DGP.Estimate(rplus)
		}
