...
foreach julia in "" julia {
  ...
  boottest tenure, `julia' ptype(equaltail) seed(987654321) stat(c) nogr  // same but bootstrap-c
  boottest tenure, `julia' ptype(equaltail) seed(987654321) stat(c) gridmin(-2) gridmax(2) nogr // same but limit graphing range
